...
@Service
public class CartesianCoordinateServiceImpl implements CartesianCoordinateService {
	

	@Override
	public String getDefinitionOfLineByTwoPoints(double x1, double y1, double x2, double y2) {
		if(x1==x2 && y1==y2) {
			return Constants.LINE_NOT_POSSIBLE_BECAUSE_OF_SAME_POINTS;
		}
		if(x1==x2) {
			return "x=" + x1;
		} else if (y1==y2) {
	...
	public String getIntersectionPoint(double x1, double y1, double x2, double y2, double x3, double y3, double x4,
		...
		if(determinant==0) {
			return Constants.LINES_WILL_NEVER_INTERSECT;
		} else {
